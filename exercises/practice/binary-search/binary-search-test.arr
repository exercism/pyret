...
fun value-in-list-of-one():
  check "finds a value in an list with one element":
    binary-search([list: 6], 6) is 0
  end
end

fun value-in-middle-of-list():
  check "finds a value in the middle of an list":
...
[list: 
  ...
  test(value-in-middle-of-list, false),
  test(value-in-beginning-of-list, false),
  test(value-at-end-of-list, false),
  test(value-in-odd-length-list, false),
  test(value-in-even-length-list, false),
  test(error-value-not-in-list, false),
  test(error-value-too-small, false),
  test(err-value-too-large, false),
  test(err-empty-list, false),
  test(err-bounds-cross, false)
